...
<head>
    ...
    <title><?= $data['post']["title"] ?> | Blog MaBagnole</title>
...
<body class="bg-white text-brand-black antialiased selection:bg-brand-orange selection:text-white">
    ...
    <article>
            ...
            <div class="container mx-auto max-w-4xl text-center">
                ...
                <div class="flex items-center justify-center gap-4">
                    <div class="w-12 h-12 bg-gray-200 rounded-full flex items-center justify-center text-xl font-black text-gray-400">
                        <?= substr($data['post']['author'], 0, 1) ?>
                    </div>
                    <div class="text-left">
                        <p class="text-xs font-bold text-gray-400 uppercase tracking-widest">Écrit par</p>
                        <p class="font-bold text-brand-black"><?= $data['post']['author'] ?></p>
                    </div>
                </div>
            </div>
        </header>
        ...
        <div class="container mx-auto max-w-5xl px-6 -mt-8">
            <div class="aspect-video rounded-3xl overflow-hidden shadow-2xl shadow-brand-black/10">
                <img src="<?= $data['post']['media'] ?>" class="w-full h-full object-cover">
            </div>
        </div>
        <div class="container mx-auto max-w-3xl px-6 py-16">
            ...
            <div class="mt-16 pt-8 border-t border-gray-100">
                ...
                <div class="flex flex-wrap gap-2">
                    ...
                    <?php foreach($data['tags'] as $tag): ?>
                        <a href="/blog/index?tag=<?= $tag["name"] ?>" class="px-4 py-2 bg-gray-50 hover:bg-brand-black hover:text-white rounded-lg text-xs font-bold transition text-gray-600">#<?= $tag["name"] ?></a>
                    <?php endforeach; ?>
                    <?php endif; ?>
                </div>
            </div>
            ...
            <div class="mt-16 pt-12 border-t border-gray-100" id="comments">
                <h3 class="text-2xl font-black text-brand-black uppercase mb-8">Commentaires (<?= count($data['comments']) ?>)</h3>
                
                <?php if(isset($_SESSION['id'])): ?>
                <form action="/comment/store" method="POST" class="mb-12 bg-gray-50 p-6 rounded-2xl">
                    <input type="hidden" name="article_id" value="<?= $data['post']['id'] ?>">
                    <div class="mb-4">
                ...
                <div class="bg-gray-50 p-6 rounded-2xl mb-12 text-center">
                    ...
                    <a href="/login" class="inline-block bg-brand-black text-white font-bold py-3 px-8 rounded-xl hover:bg-brand-orange transition">Se connecter</a>
                </div>
                <?php endif; ?>
                        ...
                        <div class="flex gap-4" id="comment-<?= $comment['commentId'] ?>">
                            ...
                            <div class="flex-1">
                                <?php if(isset($_GET['edit_comment']) && $_GET['edit_comment'] == $comment['commentId'] && isset($_SESSION['id']) && $_SESSION['id'] == $comment['user_id']): ?>
                                    <!-- Edit Form -->
                                    <form action="/comment/update" method="POST" class="bg-white border-2 border-brand-orange rounded-2xl p-6">
                                        ...
                                        <div class="flex justify-end gap-3">
                                            <a href="?article=<?= $data['post']['id'] ?>#comment-<?= $comment['commentId'] ?>" class="px-4 py-2 text-sm font-bold text-gray-500 hover:text-gray-800 transition">Annuler</a>
                                    ...
                                    <div class="bg-gray-50 rounded-2xl p-6 rounded-tl-none relative group">
                                        ...
                                        <?php if(isset($_SESSION['id']) && $_SESSION['id'] == $comment['user_id']): ?>
                                            <div class="absolute top-4 right-4 flex gap-3 opacity-0 group-hover:opacity-100 transition">
                                                <a href="?edit_comment=<?= $comment['commentId'] ?>#comment-<?= $comment['commentId'] ?>" class="text-gray-400 hover:text-brand-orange transition" title="Modifier">
                                                    <i class="fa-solid fa-pen-to-square"></i>
                                                </a>
                                                <a href="/comment/delete/<?= $comment['commentId'] ?>" class="text-gray-400 hover:text-red-500 transition" title="Supprimer" onclick="return confirm('Voulez-vous vraiment supprimer ce commentaire ?')">
            ...
            <div class="mt-16 bg-brand-black rounded-3xl p-8 md:p-12 text-center relative overflow-hidden group">
                ...
                <div class="relative z-10">
                    ...
                    <a href="/vehicle" class="inline-block bg-white text-brand-black font-bold py-4 px-8 rounded-xl hover:bg-brand-orange hover:text-white transition shadow-lg">
    ...
    <footer class="bg-white border-t border-gray-100 py-12">
        <div class="container mx-auto px-6 text-center">
            <a href="/blog" class="text-xs font-black text-gray-400 uppercase tracking-widest hover:text-brand-orange transition">
